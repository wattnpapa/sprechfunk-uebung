...
export class Uebungsleitung extends BasePDF {
    ...
    draw(): void {
        ...
        const tableBody = this.funkUebung.teilnehmerListe.map((teilnehmer: string) => {
        ...
        const alleNachrichten: { nr: number; empfaenger: string; sender: string; nachricht: string; zeit: string }[] = [];
        ...
        const lfdnrWidth = 12;
        const withCheckBox = 24;
        const senderWidth = empfaengerWidth; // gleiche Breite wie Empfänger
        const columnWidthsAll = [lfdnrWidth, empfaengerWidth, senderWidth, contentWidth - lfdnrWidth - (empfaengerWidth * 2) - withCheckBox, withCheckBox];
        // Neuen Y-Startpunkt nach der Teilnehmer-Tabelle
        this.pdf.addPage();
        const nextTableStartY = secondPageTableTopMargin;
